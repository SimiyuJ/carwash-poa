...
export default function ReceiptsPage() {
  ...
  const loadReceipts = async (branchId?: string, carwashId?: string) => {
  ...
  useEffect(() => {
    ...
    loadReceipts(userBranchId, userCarwashId);
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          const invoice = payload.new as ReceiptRecord;

          if (invoice?.carwash_id !== userCarwashId) {
            return;
          }
  ...
  const filteredReceipts = useMemo(() => {
    ...
    return receipts.filter((receipt) => {
      const invoice = receipt.invoice_number?.toLowerCase() || "";

      const customer = receipt.customer?.toLowerCase() || "";

      const plate = receipt.plate?.replace(/\s+/g, "").toLowerCase() || "";

      return (
        invoice.includes(q) ||
        customer.includes(q) ||
        plate.includes(q.replace(/\s+/g, ""))
      );
    });
  }, [receipts, search]);
