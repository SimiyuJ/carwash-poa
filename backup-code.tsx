...
========================================================= */
  ...
  const fetchPlans = async () => {
    ...
    const formatted = (data || []).map((plan: any) => ({
      ...
      washes: plan.description || "",

      benefits: [...(plan.features || []), ...(plan.perks || [])],
  ...
  const findMemberByPlate = async () => {
    ...
    try {
      ...
      if (existingMember) {
        const exhausted =
          Number(existingMember.usage) >= Number(existingMember.limit);

        const expired =
          existingMember.renewal &&
          new Date(existingMember.renewal) < new Date();

        const activeSubscription =
          existingMember.status === "active" && !exhausted && !expired;

        if (activeSubscription) {
          alert("Customer already has an active subscription.");
          return;
        }
      }
                ...
                "
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder="500"
                        className="w-full h-12 rounded-2xl border border-white/10 bg-[#111827] px-4"
                      />
                    </div>
      ...
      "
            ...
            "
                ...
                "
                        ...
                        <p className="text-zinc-400 mt-1">{member.plan}</p>
                      </div>
                    </div>
    ...
    "
                      >
                        ...
                        <div className="mt-1 text-xl font-black text-emerald-400">
                          KES {plan.amount_saved?.toLocaleString()}
                        </div>
                      </div>
                    )}
                  ...
                  <div className="pt-5 mt-6 border-t border-white/10">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <h4 className="text-xl font-bold">{plan.users}</h4>
                      </div>
